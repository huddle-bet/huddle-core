...
export function priorFor(relation, statA, statB) {
    if (relation === 'cross-game')
        return 0;
    ...
    for (const [x, y, r] of VOLUME_PAIRS) {
        if ((statA === x && statB === y) || (statA === y && statB === x)) {
            if (relation === 'same-team')
                return r;
            // The CS2 kills↔kills entry is an opponents pair: more rounds lift both.
            if (relation === 'opponents')
                return r;
        }
    }
    if (relation === 'same-team')
        return 0.15; // shared game script, mildly positive
    if (relation === 'opponents')
        return -0.05; // one team's volume mildly crowds the other's
...
const SHRINK_K = 10;
export function shrunkCorrelation(paired, prior) {
    const n = paired ? Math.min(paired.a.length, paired.b.length) : 0;
    if (!paired || n < MIN_SHARED_GAMES)
        return { r: prior, source: 'prior' };
...
export function effectiveCorrelation(r, sideA, sideB) {
    const sign = (s) => (s === 'over' ? 1 : -1);
 ...
 */
export function toPositiveDefinite(matrix) {
    let m = matrix;
    let lambda = 0;
    for (let attempt = 0; attempt < 25; attempt++) {
        if (cholesky(m))
            return { matrix: m, shrunkBy: lambda };
        lambda = lambda === 0 ? 0.05 : Math.min(1, lambda * 1.5);
        const dim = matrix.length;
        m = matrix.map((row, i) => row.map((v, j) => (i === j ? 1 : v * (1 - lambda))));
        void dim;
    }
 ...
 */
export function jointHitProbability(marginals, correlation, seed) {
    const { matrix } = toPositiveDefinite(correlation);
    const L = cholesky(matrix);
    if (!L)
        return marginals.reduce((p, m) => p * m, 1); // identity fallback — independent
...
export function correlationSignal(pairwise) {
    if (pairwise.length === 0)
        return { score: 0, label: 'independent' };
...
//# sourceMappingURL=parlay-correlation.js.map
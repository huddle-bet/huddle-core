 ...
 */
export function normalizeMarketType(type) {
    const key = type.toLowerCase().replace(/\s+/g, "_");
    // Direct match
    if (key in MARKET_TYPE_MAP)
        return MARKET_TYPE_MAP[key];
    ...
    if (fdMatch) {
        ...
        if (stat in FD_STAT_MAP)
            return FD_STAT_MAP[stat];
 ...
 */
export function isUsefulMarket(type) {

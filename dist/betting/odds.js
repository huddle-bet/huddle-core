...
export function americanToDecimal(american) {
    if (!Number.isFinite(american))
        throw new OddsError('odds must be a finite number');
    if (american >= 100)
...
export function devig(decimals) {
    if (decimals.length < 2)
        throw new OddsError('devig needs at least two prices to normalise against');
 ...
 */
export function expectedValue(trueProbability, decimal) {
 ...
 * with its own vig.
 */
export function kellyFraction(trueProbability, decimal, multiplier = 0.25) {
    const b = decimal - 1;
    if (b <= 0)
        return 0;
...
export function decimalFromAmericanString(raw) {

 ...
 */
export function playersMatch(registry, sport, name1, name2, teamIds) {
 ...
 */
function toAbbreviated(normalized) {
    const parts = normalized.split(' ');
    if (parts.length < 2)
        return null;
    if (parts[0].length <= 2)
        return null; // Already abbreviated
    return `${parts[0][0]}. ${parts.slice(1).join(' ')}`;
}
//# sourceMappingURL=player-matcher.js.map
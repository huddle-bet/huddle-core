...
export function parseWebshareList(body) {
    const urls = [];
...
export async function getProxyPool(opts = {}) {
    ...
    inFlight = (async () => {
        try {
            ...
            if (!res.ok)
                throw new Error(`webshare list HTTP ${res.status}`);
...
export function peekProxyPool() {
    return cached;
}
export function __resetProxyPoolForTests() {
    cached = null;
    inFlight = null;
}

...
import { reduceNBA, createNBAState } from './nba.js';
function initialState(eventId = 'nba-1', league = 'nba') {
    return {
        event_id: eventId,
        source_id: 'espn',
        league_id: league,
...
describe('reduceNBA', () => {
    ...
    it('status transition live → final emits game_ended feed with scores', () => {
        ...
        const scored = fold([evt({ kind: 'score' }, { homeScore: 112, awayScore: 108 })], live.state);
        const { state, feed } = fold([evt({ kind: 'status' }, { status: 'final' })], scored.state);
    ...
    it('score event with no change does not emit feed', () => {
        ...
        const { feed } = fold([evt({ kind: 'score' }, { homeScore: 50, awayScore: 48 })], first.state);
        expect(feed).toHaveLength(0);
    });
    it('clock event sets period label and clock string', () => {
        const { state } = fold([evt({ kind: 'clock' }, { period: '2', clock: '8:15' })]);
        const game = state.state.gameState;
    ...
    it('period end emits period_ended feed carrying current scores', () => {
        ...
        const { feed } = fold([evt({ kind: 'period' }, { period: '1', phase: 'end' })], scored.state);
...
//# sourceMappingURL=nba.test.js.map
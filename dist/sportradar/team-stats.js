...
function takeScalars(out, obj, prefix = '') {
...
function takeScalarsDeep(out, obj, prefix = '') {
    if (obj == null || typeof obj !== 'object' || Array.isArray(obj))
        return;
    for (const [k, v] of Object.entries(obj)) {
        if (isScalar(v))
 ...
 */
export function nflTeamStats(team) {
    const out = {};
    ...
    for (const [category, group] of Object.entries(team ?? {})) {
        ...
        if (totals)
            takeScalars(out, totals, `${category}_`);
    }
    return out;
}
/** NBA summary `{home,away}.statistics`: ~78 flat scalars, taken as-is. */
export function nbaTeamStats(statistics) {
    const out = {};
 ...
 */
export function nhlTeamStats(team) {
    const out = {};
    const statistics = team?.statistics;
    takeScalars(out, statistics?.total);
    for (const block of ['powerplay', 'shorthanded', 'evenstrength', 'penalty']) {
    ...
    if (isScalar(team?.points))
        out.score = team.points;
 ...
 */
export function mlbTeamStats(team) {
    const out = {};
    for (const k of ['runs', 'hits', 'errors']) {
        if (isScalar(team?.[k]))
            out[k] = team[k];
    }

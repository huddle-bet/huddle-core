...
import { EVENT_STATUSES, isEventStatus } from '../types/events.js';
describe('EventStatus taxonomy', () => {
    ...
    it('narrows to EventStatus', () => {
        ...
        if (!isEventStatus(raw))
            throw new Error('expected postponed to narrow');
        const narrowed = raw;
...
describe('delayed is distinct from the states it is nearest to', () => {
    it('is a member, and narrows', () => {
        ...
        const raw = 'delayed';
        if (!isEventStatus(raw))
            throw new Error('expected delayed to narrow');
        const narrowed = raw;
...
//# sourceMappingURL=event-status.test.js.map
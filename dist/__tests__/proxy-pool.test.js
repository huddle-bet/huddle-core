...
import { getProxyPool, parseWebshareList, parseCsvList, peekProxyPool, __resetProxyPoolForTests, PROXY_LIST_URL_ENV, PROXY_CSV_ENV, } from '../proxy/pool.js';
...
const body = (n) => Array.from({ length: n }, (_, i) => `10.0.0.${i}:${5000 + i}:user:pass`).join('\n');
const okResponse = (text) => ({ ok: true, status: 200, text: () => Promise.resolve(text) });
beforeEach(() => {
...
describe('resolution prefers Webshare', () => {
    ...
    it('caches within the TTL rather than fetching per request', async () => {
        ...
        await getProxyPool({ fetchImpl: spy });
        await getProxyPool({ fetchImpl: spy });
    ...
    it('refetches after the TTL', async () => {
        const spy = vi.fn(() => Promise.resolve(okResponse(body(20))));
        await getProxyPool({ fetchImpl: spy });
        await getProxyPool({ fetchImpl: spy, now: Date.now() + 61 * 60_000 });
...
describe('a bad response must not empty the pool', () => {
    ...
    it('falls back on a non-200', async () => {
        const pool = await getProxyPool({
            fetchImpl: () => Promise.resolve({ ok: false, status: 502, text: () => Promise.resolve('') }),
        });
        expect(pool.source).toBe('env');
    });
...
//# sourceMappingURL=proxy-pool.test.js.map
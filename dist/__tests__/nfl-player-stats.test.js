...
import { NFL_EXPECTED_PLAYERS, NFL_TEAM_STATISTICS, } from '../sportradar/__fixtures__/nfl-team-statistics.js';
/**
...
describe('nflTeamPlayerStats matches huddle-data', () => {
    ...
    it.each(NFL_EXPECTED_PLAYERS.map((p) => [p.name, p]))('%s', (name, expected) => {
...
describe('the vocabulary is the one projections.ts reads', () => {
    ...
    it('does not emit the ambiguous compact keys', () => {
        ...
        const all = nflTeamPlayerStats(NFL_TEAM_STATISTICS).flatMap((p) => Object.keys(p.stats));
        for (const key of ['YDS', 'TD', 'REC', 'CMP'])
...
describe('extra_points nests one level deeper and is still flattened', () => {
    ...
    it('does not flatten a nested group that holds no players array', () => {
        ...
        const team = { efficiency: { redzone: { attempts: 1, successes: 1, pct: 100 } } };
        expect(nflTeamPlayerStats(team)).toEqual([]);
    });
});
//# sourceMappingURL=nfl-player-stats.test.js.map
 ...
 */
export function mlbStarterIds(team: unknown): Set<string> {
  const lineup = (team as { lineup?: unknown } | null | undefined)?.lineup;
  if (!Array.isArray(lineup)) return new Set();
  const ids = new Set<string>();
  for (const entry of lineup) {
    const e = entry as { id?: unknown; inning?: unknown };
    if (e?.inning === 0 && typeof e.id === 'string' && e.id.length > 0) ids.add(e.id);
  }
  return ids;
}

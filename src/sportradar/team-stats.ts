...
/** Scalars of `obj` into `out`, keys prefixed. Non-scalars skipped. */
function takeScalars(out: Stats, obj: unknown, prefix = ''): void {
...
/** Scalars of `obj` plus one nested level, `parent_child` keys. */
function takeScalarsDeep(out: Stats, obj: unknown, prefix = ''): void {
  if (obj == null || typeof obj !== 'object' || Array.isArray(obj)) return;
  for (const [k, v] of Object.entries(obj as Record<string, unknown>)) {
    if (isScalar(v)) out[`${prefix}${k}`] = v;
    else takeScalars(out, v, `${prefix}${k}_`);
  }
}
 ...
 */
export function nflTeamStats(team: any): Stats {
  const out: Stats = {};
  takeScalars(out, team?.summary);
  takeScalars(out, team?.first_downs, 'first_downs_');
  takeScalars(out, team?.touchdowns, 'touchdowns_');
  takeScalarsDeep(out, team?.efficiency, 'efficiency_');
  for (const [category, group] of Object.entries(team ?? {})) {
    const totals = (group as any)?.totals;
    if (totals) takeScalars(out, totals, `${category}_`);
  }
  return out;
}

/** NBA summary `{home,away}.statistics`: ~78 flat scalars, taken as-is. */
export function nbaTeamStats(statistics: any): Stats {
  const out: Stats = {};
  takeScalars(out, statistics);
  return out;
}
 ...
 */
export function nhlTeamStats(team: any): Stats {
  const out: Stats = {};
  const statistics = team?.statistics;
  takeScalars(out, statistics?.total);
  for (const block of ['powerplay', 'shorthanded', 'evenstrength', 'penalty'] as const) {
    takeScalars(out, statistics?.[block], `${block}_`);
  }
  // Not `?? 0`: an absent `points` is "the provider did not tell us", and a 0 there is a
  // real shutout. The distinction is the whole of `check:score-of-record`, which cannot
  // read a fabricated 0 as anything but a score.
  if (isScalar(team?.points)) out.score = team.points;
  takeScalars(out, team?.shootout, 'shootout_');
  return out;
}
 ...
 */
export function mlbTeamStats(team: any): Stats {
  const out: Stats = {};
  for (const k of ['runs', 'hits', 'errors'] as const) {
    if (isScalar(team?.[k])) out[k] = team[k];
  }
  takeScalarsDeep(out, team?.statistics?.hitting?.overall, 'hitting_');
  takeScalarsDeep(out, team?.statistics?.pitching?.overall, 'pitching_');
  takeScalarsDeep(out, team?.statistics?.fielding?.overall, 'fielding_');
  return out;
}

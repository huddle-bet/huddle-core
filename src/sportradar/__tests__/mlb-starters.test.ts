...
type Team = {
  ...
  starting_pitcher: { id: string; full_name: string };
};

const sides: Array<[string, Team]> = [['home', SUMMARY.game.home], ['away', SUMMARY.game.away]];
...
describe('mlbStarterIds', () => {
  ...
  it('excludes a substitute who holds two entries', () => {
    ...
    for (const id of twice) expect(ids.has(id)).toBe(false);
  });

 ...
 */
const sport = (k: 'mlb' | 'nba' | 'nhl') => SUMMARY_SCORING[k] as unknown;
...
describe('summaryLineScore', () => {
  ...
  it('sorts hockey periods, which the provider sends backwards', () => {
    const raw = (SUMMARY_SCORING.nhl as unknown as { home: { scoring: ReadonlyArray<{ number: number }> } }).home.scoring;
  ...
  it('does not collide overtime with the first quarter', () => {
    const raw = (SUMMARY_SCORING.nba as unknown as { home: { scoring: ReadonlyArray<{ number: number }> } }).home.scoring;

 ...
 */
const UNACCENT_NON_DECOMPOSABLE: Record<string, string> = {
 ...
 */
export function stripOrgSuffix(normalized: string): string | null {
  const parts = normalized.split(' ');
  if (parts.length < 2) return null;
  if (!ORG_SUFFIXES.has(parts[parts.length - 1].toLowerCase())) return null;

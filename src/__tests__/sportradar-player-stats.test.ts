 ...
 * one completed game per sport.
 *
 * `expected` is **not** hand-written and does not come from this implementation. Each
 * value is the output of huddle-data's shipped `normalizeNbaGame` / `normalizeNhlGame` /
 * `normalizeMlbGame` run end-to-end on that same payload — the flat vocabulary that is
 * already in `player_game_stats` today and that `huddle-engine/projections.ts` reads.
 *
 * That is the whole point. These functions exist so huddle-live can write the same shape
...
describe.each(['nba', 'nhl', 'mlb'] as const)('%s matches huddle-data byte for byte', (sport) => {
  ...
  it.each(pairs.map((p) => [p.player, p] as const))('%s', (_name, pair) => {
...
describe('the vocabulary is the one projections.ts reads', () => {
  ...
  it.each([
    ...
    for (const k of keys) expect(Object.keys(flat), `${sport} is missing ${k}`).toContain(k);
  });
});
...
describe('missing values become zero, not undefined', () => {
  it('nhl fills every tracked key', () => {
    ...
    for (const v of Object.values(flat)) expect(v).not.toBeUndefined();

...
describe('canonicalEventId', () => {
  ...
  */
  ...
  it('throws when teamIdA is missing', () => {
    expect(() => canonicalEventId({ ...baseKey, teamIdA: '' })).toThrow(
      /requires both team IDs/,
    );
  });

  it('throws when teamIdB is missing', () => {
    expect(() => canonicalEventId({ ...baseKey, teamIdB: '' })).toThrow(
      /requires both team IDs/,
    );
  });
  ...
  it('throws on an unparseable ISO timestamp', () => {
    expect(() =>
      canonicalEventId({ ...baseKey, startTime: 'not-a-date' }),

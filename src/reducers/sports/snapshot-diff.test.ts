...
import { snapshotDiff, type SportsSnapshot, type SnapshotContext } from './common.js';
...
function snap(over: Partial<SportsSnapshot> = {}): SportsSnapshot {
  return {
    status: 'live',
    homeScore: 0,
    awayScore: 0,
...
describe('snapshotDiff', () => {
  ...
  it('status transition emits status event', () => {
    ...
    expect((statusEvt as any).status).toBe('live');
  });
  ...
  it('score change emits score event with delta + scorer', () => {
    ...
    expect((scoreEvt as any).homeScore).toBe(13);
    expect((scoreEvt as any).scorer).toBe('home');
    expect((scoreEvt as any).delta).toBe(3);
  });
  ...
  it('period change emits end-of-period event for the outgoing period', () => {
    ...
    expect((periodEvt as any).period).toBe('1');
    expect((periodEvt as any).phase).toBe('end');
  });
  ...
  it('possession flip emits possession event', () => {
    ...
    expect((posEvt as any).team).toBe('away');
  });

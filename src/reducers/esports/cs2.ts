...
interface CS2Player {
  ...
  bombPlanted: number;
  bombDefused: number;
  mvps: number;              // round MVPs across the series
}
...
export function createCS2State(): CS2GameState {
  return {
    mapNumber: null,
    mapName: null,
    roundNumber: 0,
...
export function makeEmptyCS2Player(p: any): CS2Player {
  return {
    id: String(p.id || ''),
    name: p.name || '',
...
export function ensureCS2Player(state: CS2GameState, playerPayload: any): void {
  if (!playerPayload) return;
  const id = String(playerPayload.id);
  if (!state.players[id]) {
    state.players[id] = makeEmptyCS2Player(playerPayload);
  }
  if (playerPayload.name) state.players[id].name = playerPayload.name;
  if (playerPayload.side) state.players[id].side = playerPayload.side;
  if (playerPayload.teamId) state.players[id].teamId = String(playerPayload.teamId);
}

function applyCS2FullState(state: CS2GameState, payload: any): void {
  if (payload.mapNumber != null) state.mapNumber = payload.mapNumber;
  if (payload.roundNumber != null) state.roundNumber = payload.roundNumber;

  if (payload.ct?.id) state.sides[String(payload.ct.id)] = 'CT';
  if (payload.terrorist?.id) state.sides[String(payload.terrorist.id)] = 'Terrorist';

  for (const sideKey of ['ct', 'terrorist'] as const) {
...
export function reduceCS2(
  prev: LiveStateRow,
  msg: EsportsLiveEvent,
): ReducerResult {
  const gameState: CS2GameState = (prev.state.gameState as CS2GameState | undefined) ?? createCS2State();
  ...
  const stateRow: LiveStateRow = { ...prev, sort_index: msg.sortIndex };
  ...
  if (!payload) return { state: stateRow, feed };
  ...
  if (name === 'map_ended') {
    ...
    const scores = (payload.participants || [])
      .map((p: any) => `${teams[String(p.id)]?.name || p.id} ${p.roundsWon}`)
      .join(' — ');
  ...
  if (name === 'kill') {
    ...
    if (gameState.players[killerId]) {
      ...
      if (payload.headshot) gameState.players[killerId].headshots++;
    }
    ...
    const flags: string[] = [];
    if (payload.headshot) flags.push('HEADSHOT');
    if (payload.noScope) flags.push('NO SCOPE');
    if (payload.throughSmoke) flags.push('THROUGH SMOKE');
    if (payload.penetrated) flags.push('WALLBANG');
    if (payload.whileBlinded) flags.push('BLIND');
    ...
    for (const [tid, side] of Object.entries(gameState.sides)) {
      ...
      if (side === 'CT') ctScore = rw;
      else if (side === 'Terrorist') tScore = rw;
    }
    ...
    feed.push(makeFeedRow(
      feedBase, msg.sortIndex, 'kill',
      isSpecial ? 'high' : 'low',
      {
  ...
  if (name === 'assist') {
    ...
    if (gameState.players[id]) gameState.players[id].assists++;
  }

  if (name === 'flash_assist') {
    ensureCS2Player(gameState, payload.assister?.player);
    const id = String(payload.assister?.player?.id);
    if (gameState.players[id]) gameState.players[id].flashAssists++;
  }
  ...
  if (name === 'equipment_state') {
    for (const p of payload.players || []) {
      ...
      if (!gameState.players[id]) gameState.players[id] = makeEmptyCS2Player(p);
  ...
  if (
    (type === 'occurrence' && name === 'full_state')
    || type === 'full_state'
    ...
    const clutchDeltas: Array<{ playerId: string; playerName: string; teamId: string; side?: string; agent?: string; total: number }> = [];
    const prevClutch = gameState._prevClutchWins ?? {};
    const payloadPlayers = [
      ...(payload.players ?? []),
      ...((payload.ctPlayers ?? []) as any[]),
      ...((payload.tPlayers ?? []) as any[]),
    ];
    for (const p of payloadPlayers) {
      if (!p || typeof p.clutchWins !== 'number') continue;

...
export interface LoLGameState {
  ...
  lastObjective: { type: string; teamId: string; detail?: string } | null;
  ...
  _killChains?: Record<string, { lastGameTime: number; count: number }>;
...
export function createLoLState(): LoLGameState {
  return {
    mapNumber: null,
    gameTime: 0,
    phase: 'warmup',
...
function ensureLoLPlayer(state: LoLGameState, playerPayload: any): void {
...
function normalizeDragon(raw: string): string {
  if (!raw) return 'Unknown';
  return raw.charAt(0).toUpperCase() + raw.slice(1).toLowerCase();
}

function applyLoLFullState(state: LoLGameState, payload: any): void {
  state.gameTime = payload.gameTime || state.gameTime;

  for (const sideKey of ['blueTeam', 'redTeam'] as const) {
...
export function reduceLoL(
  prev: LiveStateRow,
  msg: EsportsLiveEvent,
): ReducerResult {
  const gameState: LoLGameState = (prev.state.gameState as LoLGameState | undefined) ?? createLoLState();
  ...
  if (name === 'kill') {
    ...
    for (const a of payload.assists || []) {
      ...
      if (gameState.players[aid]) gameState.players[aid].assists++;
    }
    if (gameState.players[killerId]) {
      gameState.players[killerId].kills++;
      const teamId = gameState.players[killerId].teamId;
      if (gameState.teams[teamId]) gameState.teams[teamId].totalKills++;
    }
    ...
    const assistNames = (payload.assists || []).map((a: any) => a?.name || a?.player?.name || 'Unknown');
    const subtextParts: string[] = [];
    if (killerChamp) subtextParts.push(killerChamp);
    if (assistNames.length) subtextParts.push(`Assists: ${assistNames.join(', ')}`);
  ...
  if (name === 'tower_destroyed') {
    ...
    if (gameState.teams[teamId]) gameState.teams[teamId].towers++;
  ...
  if (name === 'inhibitor_destroyed') {
    gameState.gameTime = payload.gameTime || gameState.gameTime;
    const teamId = String(payload.teamId);
    if (gameState.teams[teamId]) gameState.teams[teamId].inhibitors++;
  ...
  if (name === 'dragon_slain') {
    ...
    feed.push(makeFeedRow(
      feedBase, msg.sortIndex, 'dragon_slain',
      normDrg === 'Elder' ? 'critical' : 'high',
      {
  ...
  if (name === 'baron_slain') {
    gameState.gameTime = payload.gameTime || gameState.gameTime;
    const teamId = String(payload.teamId);
    if (gameState.teams[teamId]) gameState.teams[teamId].barons++;

 ...
 */
export async function scrapeApiKey(signal?: AbortSignal): Promise<string> {
  ...
  if (!res.ok) throw new Error(`lolesports.com HTTP ${res.status} — cannot harvest key`);
...
export function _resetLolesportsApiKeyState(): void {

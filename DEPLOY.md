...
| `SPORTRADAR_API_KEY` | huddle-data, huddle-live, **huddle-api** | NBA/NFL/NHL/MLB stats + live. huddle-api reads it too; the row omitted that until 2026-09-08. |
| `SPORTRADAR_ACCESS_LEVEL` | huddle-data, huddle-live, huddle-api | `trial` or `production` — appears in the API path, so it is not cosmetic. **The three values differ deliberately, measured 2026-09-06:** huddle-live is `production`, the other two are `trial`, so the trial credits are spent first. Flipping the two `trial` values spends paid credits while trial ones expire unused. See ENG-221, whose body still describes the pre-2026-09-06 state. Retires when the trial credits are exhausted or expired. |
| `SPORTRADAR_PUSH_KEY` | huddle-live | Push-feed key (separate from REST key on Sportradar's side) |
| `SPORTRADAR_NBA` / `SPORTRADAR_NHL` / `SPORTRADAR_MLB` / `SPORTRADAR_NFL` | huddle-live | **League on-switches, and the only thing that decides whether a league's live adapter runs.** `cli.ts` gates on strict `=== '1'`; any other value, including unset, starts the process with that adapter never registered and nothing logged about its absence. Losing one reads as an empty calendar rather than as an outage. Undocumented here until 2026-09-08 (ENG-842) and present only in `huddle-live/.env.example`. |
...
| `THUNDERPICKS_API_KEY` | *nobody* | Esports book, **and no service reads it any more — it can be unset.** huddle-odds' Thunderpick client went on 2026-08-05 when the key stopped authenticating (401 on the documented header and unauthenticated alike, ENG-543) and CS2 was already covered by PrizePicks and Underdog; the legacy alias `TP_APIKEY` went with it. huddle-engine's direct fetch went with ENG-438, which **deleted** that path rather than rerouting it — `huddle-engine#204`, "the last outbound HTTP in huddle-engine". Measured on master 2026-09-20: 0 Thunderpick files under `src` in either repo. |
